...
void sha256_final(SHA256_CTX *ctx, uint8_t hash_out[32]) {
    ...
    sha256_transform(ctx, ctx->buffer);
    for (i=0; i<8; ++i) {
        hash_out[i*4]   = (ctx->state[i] >> 24) & 0xff;
        hash_out[i*4+1] = (ctx->state[i] >> 16) & 0xff;
        hash_out[i*4+2] = (ctx->state[i] >> 8) & 0xff;
        hash_out[i*4+3] = ctx->state[i] & 0xff;
    }
}
